...
import { EditCourseExtensionActivitiesDataFormProps } from "./editCourseExtensionActivitiesDataForm.interface";

const UPDATE_COURSE_EXTENSION_ACTIVITIES_DATA_SUCCESS_MESSAGE =
  "Registro de atividades de extensão do curso foi atualizado!";

const UPDATE_COURSE_EXTENSION_ACTIVITIES_DATA_ERROR_MESSAGES = {
  [HTTP_STATUS.forbidden]:
    "Você não tem permissão para realizar essa operação!",
  [HTTP_STATUS.notFound]:
    "O registro de atividades de extensão do curso informado não foi encontrado!",
  [HTTP_STATUS.badRequest]: "Ocorreu algum erro ao atualizar as informações!",
} as const;

export function EditCourseExtensionActivitiesDataForm({
  courseExtensionActivitiesData,
}: EditCourseExtensionActivitiesDataFormProps) {
  ...
  const formMethods = useForm<RegisterCourseExtensionActivitiesDataDto>({
    ...
    defaultValues: {
      ...
      year: String(courseExtensionActivitiesData.year) as unknown as number,
  ...
  async function sendCourseExtensionActivitiesData(
    ...
    try {
      ...
      await CourseExtensionActivitiesDataCSService.update(
        courseExtensionActivitiesData.id,
        registerCourseExtensionActivitiesDataDto
      );

      Toast.success(UPDATE_COURSE_EXTENSION_ACTIVITIES_DATA_SUCCESS_MESSAGE);
    } catch (error) {
      ExceptionHandler.handle({
        error,
        errorMap: UPDATE_COURSE_EXTENSION_ACTIVITIES_DATA_ERROR_MESSAGES,
    ...
    <FormProvider {...formMethods}>
      <form onSubmit={handleSubmit(sendCourseExtensionActivitiesData)}>
        <Card>
          <CardHeader>
            <CardTitle>
              Atualize as informações de atividades de extensão do curso de{" "}
              {activeCourse?.name}
            </CardTitle>
          </CardHeader>
