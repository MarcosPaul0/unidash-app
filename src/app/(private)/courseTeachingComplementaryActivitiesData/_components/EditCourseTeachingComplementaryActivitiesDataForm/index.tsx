...
import { EditCourseTeachingComplementaryActivitiesDataFormProps } from "./editCourseTeachingComplementaryActivitiesDataForm.interface";

const UPDATE_COURSE_TEACHING_COMPLEMENTARY_ACTIVITIES_DATA_SUCCESS_MESSAGE =
  "Registro de atividades complementares de ensino do curso foi atualizado!";

const UPDATE_COURSE_TEACHING_COMPLEMENTARY_ACTIVITIES_DATA_ERROR_MESSAGES = {
  [HTTP_STATUS.forbidden]:
    "Você não tem permissão para realizar essa operação!",
  [HTTP_STATUS.notFound]:
    "O registro de atividades complementares de ensino do curso informado não foi encontrado!",
  [HTTP_STATUS.badRequest]: "Ocorreu algum erro ao atualizar as informações!",
} as const;

export function EditCourseTeachingComplementaryActivitiesDataForm({
  courseTeachingComplementaryActivitiesData,
}: EditCourseTeachingComplementaryActivitiesDataFormProps) {
  ...
  const formMethods =
    useForm<RegisterCourseTeachingComplementaryActivitiesDataDto>({
      ...
      defaultValues: {
        ...
        year: String(
          courseTeachingComplementaryActivitiesData.year
  ...
  async function sendCourseTeachingComplementaryActivitiesData(
    ...
    try {
      ...
      await CourseTeachingComplementaryActivitiesDataCSService.update(
        courseTeachingComplementaryActivitiesData.id,
        registerCourseTeachingComplementaryActivitiesDataDto
      );

      Toast.success(
        UPDATE_COURSE_TEACHING_COMPLEMENTARY_ACTIVITIES_DATA_SUCCESS_MESSAGE
      );
    } catch (error) {
      ExceptionHandler.handle({
        error,
        errorMap:
          UPDATE_COURSE_TEACHING_COMPLEMENTARY_ACTIVITIES_DATA_ERROR_MESSAGES,
    ...
    <FormProvider {...formMethods}>
      ...
      >
        <Card>
          <CardHeader>
            <CardTitle>
              Atualize as informações de atividades complementares de ensino do
