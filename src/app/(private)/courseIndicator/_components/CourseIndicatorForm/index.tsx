...
import {
  CourseIndicatorFormData,
  CourseIndicatorFormProps,
} from "./courseIndicatorForm.interface";
import { FormFile } from "@unidash/components/FormFile";

export function CourseIndicatorForm({ title }: CourseIndicatorFormProps) {
  const { control, handleSubmit } = useFormContext<CourseIndicatorFormData>();

  async function sendCourseIndicator() {
    // TODO integrar com backend
  }

  return (
    <form onSubmit={handleSubmit(sendCourseIndicator)}>
      <Card>
        <CardHeader>
        ...
        <CardContent className="flex flex-col gap-8">
          ...
          <div className="flex gap-8 w-full">
            ...
            <FormInput
              control={control}
              name="removedDepartureOfStudents"
              type="number"
