...
import {
  ...
  courseSearchComplementaryActivitiesDataDtoSchema,
...
import { EditCourseSearchComplementaryActivitiesDataFormProps } from "./editCourseSearchComplementaryActivitiesDataForm.interface";

const UPDATE_COURSE_SEARCH_COMPLEMENTARY_ACTIVITIES_DATA_SUCCESS_MESSAGE =
  "Novo registro de atividades complementares de pesquisa do curso foi atualizado!";

const UPDATE_COURSE_SEARCH_COMPLEMENTARY_ACTIVITIES_DATA_ERROR_MESSAGES = {
  [HTTP_STATUS.forbidden]:
    "Você não tem permissão para realizar essa operação!",
  [HTTP_STATUS.notFound]:
    "O registro de atividades complementares de pesquisa do curso informado não foi encontrado!",
  [HTTP_STATUS.badRequest]: "Ocorreu algum erro ao atualizar as informações!",
} as const;

export function EditCourseSearchComplementaryActivitiesDataForm({
  courseSearchComplementaryActivitiesData,
}: EditCourseSearchComplementaryActivitiesDataFormProps) {
  const { activeCourse } = useCourseStore();

  const formMethods =
    useForm<RegisterCourseSearchComplementaryActivitiesDataDto>({
      resolver: zodResolver(courseSearchComplementaryActivitiesDataDtoSchema),
      defaultValues: {
        semester: courseSearchComplementaryActivitiesData.semester,
        year: String(
          courseSearchComplementaryActivitiesData.year
  ...
  async function sendCourseSearchComplementaryActivitiesData(
    ...
    try {
      ...
      await CourseSearchComplementaryActivitiesDataCSService.update(
        courseSearchComplementaryActivitiesData.id,
        registerCourseSearchComplementaryActivitiesDataDto
      );

      Toast.success(
        UPDATE_COURSE_SEARCH_COMPLEMENTARY_ACTIVITIES_DATA_SUCCESS_MESSAGE
      );
    } catch (error) {
      ExceptionHandler.handle({
        error,
        errorMap:
          UPDATE_COURSE_SEARCH_COMPLEMENTARY_ACTIVITIES_DATA_ERROR_MESSAGES,
    ...
    <FormProvider {...formMethods}>
      ...
      >
        <Card>
          <CardHeader>
            <CardTitle>
              Atualize as informações de atividades complementares de pesquisa
