...
import { DeleteCourseRegistrationLockDataDialogProps } from "./deleteCourseRegistrationLockDataDialog.interface";
...
const DELETE_COURSE_REGISTRATION_LOCK_DATA_SUCCESS_MESSAGE =
  "Registro de dados de trancamentos do curso removido!";

const DELETE_COURSE_REGISTRATION_LOCK_DATA_ERROR_MESSAGES = {
  ...
  [HTTP_STATUS.badRequest]:
    "Não foi possível excluir o registro de trancamentos do curso!",
};

export function DeleteCourseRegistrationLockDataDialog({
  courseRegistrationLockDataId: courseRegistrationLockDataId,
}: DeleteCourseRegistrationLockDataDialogProps) {
  ...
  async function handleDeleteCourse(event: FormEvent) {
    try {
      ...
      await CourseRegistrationLockDataCSService.delete(
        courseRegistrationLockDataId
      );

      Toast.success(DELETE_COURSE_REGISTRATION_LOCK_DATA_SUCCESS_MESSAGE);

      router.refresh();
    } catch (error) {
      ExceptionHandler.handle({
        error,
        errorMap: DELETE_COURSE_REGISTRATION_LOCK_DATA_ERROR_MESSAGES,
  ...
  return (
    <Dialog>
      <Tooltip>
        ...
        <TooltipContent>
          Excluir registro de trancamentos do curso
        </TooltipContent>
      </Tooltip>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={handleDeleteCourse}>
          <DialogHeader>
            ...
            <DialogDescription>
              Você tem certeza que deseja excluir o registro de trancamentos do
